...
export const refresh = (refresh_token: string) => apiClient(endpoints.auth.refresh, { method: "POST", body: JSON.stringify({ refresh_token }) });
export const logout = (refresh_token: string) => apiClient(endpoints.auth.logout, { method: "POST", body: JSON.stringify({ refresh_token }) });
